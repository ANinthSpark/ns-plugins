...
function reisepassMenu(ply)
    local plyChar = ply:getChar()
    if ply == nil or plyChar == nil then return end
    ...
    passMenuRightButton.DoClick = function()
        ...
        for indx, _prop in pairs(reisepass.InfoTable) do
            reisepassMenu_createLabel(passMenuFrame, indx, passInfoUnits, plyChar)
        end
...
function reisepassMenu_createLabel(frame, labelIndex, passInfoUnits, plyChar)
    if frame == nil or labelIndex == nil or plyChar == nil or passInfoUnits == nil then return end
    ...
    passMenuPlyLabel:SetText(plyChar:getData(reisepass.InfoTable[labelIndex]) ..(passInfoUnits[labelIndex] or ""))
